...
| 🔍 **排障** | 403 / 429、Cloudflare challenge、错模型、不回复、JSON 写错、session 锁、插件异常、`network connection error` / transport 误配 |
...
- **诊断与回归验证**：给出 403 / 429、Cloudflare challenge、错模型、不回复、配置写死、JSON 配错、路由异常、`LLM request failed: network connection error` 等排查路径
- **Skill 自检与维护闭环**：新增自检脚本与章节收口规则，避免 references 漂移、路径写死和编码问题
...
| `references/provider-network-errors.md` | `network connection error`、`openai-codex-responses`、通用 `/v1` relay 误配诊断 |
...
| `scripts/audit_skill.py` | 自检引用、编码、硬编码路径与旧残留词 |
...
- `summaryModel` 单独压到便宜模型更合适，当前已验证 `codex/gpt-5.3-codex` 更稳
- 当前环境已升级并验证到 `v0.6.1`
...
"plugins": {
  ...
  "entries": {
    "lossless-claw": {
      "enabled": true,
      "config": {
        "summaryModel": "codex/gpt-5.3-codex"
      }
    }
  }
}
...
- `summaryModel` 单独压到便宜模型更合适，当前已验证 `codex/gpt-5.3-codex` 更稳
- 如果近期遇到长会话压缩后历史重复、上下文重放不干净、ACK/heartbeat 污染 compaction，优先升级到 `v0.6.1`
...
- **[更新] OpenClaw 官方版本说明同步到 `v2026.4.2`**
  ...
  - 补充 `Task Flow`、`before_agent_reply`、compaction 配置一致性、provider transport hardening、loopback / exec 修复等 operator-facing 变化

- **[更新] lossless-claw 说明同步到 `v0.6.1`**
  - 当前环境已升级并验证到 `v0.6.1`
  - README 与 reference 同步补充升级命令：`openclaw plugins update lossless-claw`
  - 将 `summaryModel` 推荐值从 `codex/gpt-5.2` 调整为当前环境更稳定的 `codex/gpt-5.3-codex`
  - 补充适合优先排查的场景：长会话压缩后历史重复、上下文重放不干净、heartbeat / ACK 污染 compaction

- **[新增] provider / network error 专项排障文档**
  - 新增 `references/provider-network-errors.md`
  - 明确 `LLM request failed: network connection error` 不能直接等同于真实网络断开
  - 明确普通 OpenAI-compatible `/v1` relay 不应默认套用 `openai-codex-responses`

- **[新增] skill 自检脚本与维护闭环**
  - 新增 `scripts/audit_skill.py`
  - 可自动检查：引用文件存在、reference 可发现、UTF-8 编码、无个人路径写死、无旧残留词
  - 同步 README 快速导航与 skill 维护规则
...
- **[新增] lossless-claw 上下文引擎参考文档**
  ...
  - 补充实际配置经验：推荐与 `memory-lancedb-pro` 并用，`summaryModel` 单独压到 `codex/gpt-5.3-codex`
